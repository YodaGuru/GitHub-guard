...
async function getConfig(context) {
    ...
    return {
        minStars: s['minStars'] ?? 5,
        minAgeDays: s['minAgeDays'] ?? 30,
        checkStars: s['checkStars'] ?? true,
        checkAge: s['checkAge'] ?? true,
        checkLicense: s['checkLicense'] ?? true,
        checkSecurityPolicy: s['checkSecurityPolicy'] ?? true,
        checkSignedCommits: s['checkSignedCommits'] ?? true,
        checkOrgOwner: s['checkOrgOwner'] ?? true,
        minScoreToPass: s['minScoreToPass'] ?? 3,
        postAuditComment: s['postAuditComment'] ?? true,
        scanPosts: s['scanPosts'] ?? true,
        scanPostEdits: s['scanPostEdits'] ?? true,
        scanComments: s['scanComments'] ?? true,
        silentHighScore: s['silentHighScore'] ?? false,
        warnOnSudoScripts: s['warnOnSudoScripts'] ?? true,
        enableNuclearOption: s['enableNuclearOption'] ?? true,
        removeOnImpersonation: s['removeOnImpersonation'] ?? true,
        removeOnZeroDaySudoScript: s['removeOnZeroDaySudoScript'] ?? true,
        removeOnBlacklist: s['removeOnBlacklist'] ?? true,
        customRemovalComment: s['customRemovalComment'] ?? '',
        impersonationKeywords: parseList(s['impersonationKeywords'] ?? 'lastpass,notion,metamask,ledger,malwarebytes,passbolt,proton'),
        exemptMods: s['exemptMods'] ?? true,
...
// CORE SCAN FUNCTION
// ─────────────────────────────────────────────────────────────────────────────
async function scanForGitHub(text, id, authorName, postFlair, context, bypassCache = false) {
    let cfg;
    try {
    ...
    if (safeAuthor.toLowerCase() === 'github-guard')
        return;
    ...
    try {
        ...
        const hasInstallScript = Array.isArray(contentsData) &&
            contentsData.some((file) => ['install.sh', 'setup.sh', 'install.py', 'setup.py', 'configure'].includes(file.name.toLowerCase()));
        ...
        const isOldEnough = Date.now() - new Date(data.created_at).getTime() > cfg.minAgeDays * 86400000;
        if (cfg.checkStars) {
        ...
        const isImpersonating = cfg.removeOnImpersonation &&
            cfg.impersonationKeywords.some((kw) => repoNameLower.includes(kw)) &&
            data.owner.type !== 'Organization';
        const isUltraNewRisk = cfg.removeOnZeroDaySudoScript &&
            !isOldEnough &&
            hasInstallScript &&
            data.stargazers_count === 0;
        const isKnownThreat = cfg.enableNuclearOption && !isTrustedOrg && (isImpersonating || isUltraNewRisk);
        ...
        const riskWarning = cfg.warnOnSudoScripts && hasInstallScript
        ...
        if (isKnownThreat && !isApproved) {
            ...
            const defaultRemovalText = `🛡️ **GitHub Guard: Malicious Pattern Detected**\n\n` +
        ...
        else if ((cfg.postAuditComment && !(cfg.silentHighScore && score >= cfg.minScoreToPass)) ||
            score < cfg.minScoreToPass) {
            const scoreLine = score < cfg.minScoreToPass
            ...
            const commentText = `🔍 **GitHub Guard: Trust Report**\n\n` +
        ...
        await context.redis.set(cacheKey, cachedPayload, { expiration: 3600 });
    }
    catch (e) {
...
Devvit.addTrigger({
    ...
    onEvent: async (event, context) => {
        ...
        if (!cfg.scanPosts || !cfg.scanPostEdits)
            return;
        const post = await context.reddit.getPostById(event.post.id);
        await scanForGitHub(`${post.title} ${post.url ?? ''} ${post.body ?? ''}`, event.post.id, post.authorName, post.flair?.text ?? undefined, context, true);
    },
});
Devvit.addTrigger({
    event: 'PostCreate',
    onEvent: async (event, context) => {
        ...
        if (!cfg.scanPosts)
            return;
        const post = await context.reddit.getPostById(event.post.id);
        await scanForGitHub(`${post.title} ${post.url ?? ''} ${post.body ?? ''}`, event.post.id, post.authorName, post.flair?.text ?? undefined, context);
    },
});
Devvit.addTrigger({
    event: 'CommentCreate',
...
// ─────────────────────────────────────────────────────────────────────────────
async function logScanResult(context, entry) {
    try {
        const key = `gh_log_${entry.timestamp}_${entry.owner}_${entry.repo}`;
        await context.redis.set(key, JSON.stringify(entry), { expiration: 604800 });
        const indexRaw = await context.redis.get('gh_log_index').catch(() => null);
        const index = indexRaw ? JSON.parse(indexRaw) : [];
        index.unshift(key);
        if (index.length > 50)
            index.splice(50);
        await context.redis.set('gh_log_index', JSON.stringify(index), { expiration: 604800 });
    }
    catch (e) {
...
async function rebuildWikiDashboard(context) {
    try {
        const subreddit = await context.reddit.getCurrentSubreddit();
        const indexRaw = await context.redis.get('gh_log_index').catch(() => null);
        const index = indexRaw ? JSON.parse(indexRaw) : [];
        const entries = (await Promise.all(index.map(async (k) => {
        ...
        const warned = entries.filter((e) => e.action === 'reported').length;
        const passed = entries.filter((e) => e.action === 'skipped').length;
        const rows = entries
            .map((e) => {
...
Devvit.addMenuItem({
    ...
    onPress: async (event, context) => {
        ...
        await scanForGitHub(`${post.title} ${post.url ?? ''} ${post.body ?? ''}`, event.targetId, post.authorName, post.flair?.text ?? undefined, context);
